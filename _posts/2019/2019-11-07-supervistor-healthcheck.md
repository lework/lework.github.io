...
- http 方式的检测是通过 `httplib` 库进行探测，目标返回状态码为`200`时，视为检测正常;
- tcp 方式的检测是通过 `socket` 探测端口是否能连接上，连接正常视为检测正常；
- cpu 方式检测是通过系统的 `ps`命令获取到进程的 `cpu` 使用率, 不超过给定的阈值时视为检测正常；
- mem 方式检测是通过系统的 `ps` 命令获取到进程的 `mem` 使用, 不超过给定的阈值时视为检测正常；

动作

- restart, 重启进程，通过 `supervistor`的 api 接口进行关闭，启动进程。
- exec, 执行命令，通过 `subprocess.popen`执行命令。
- email, 发送邮件通知，通过 `smtplib` 发送邮件。
- wechat, 发送企业微信通知，通过企业微信 api 接口发送通知信息。

## 实现步骤

### 实现脚本

> 源文件[supervisor_healthCheck](https://github.com/lework/script/blob/master/python/supervisor_healthCheck.py)
...
> 配置文件放置在与脚本`同级目录`下, 以`config.yaml`命名
...
通过 `/var/log/supervisor/healthCheck.log` 可以查看脚本的检测情况
